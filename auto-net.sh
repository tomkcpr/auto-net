...
function nmap-subnet () {
	...
	for IPENTRY in $( cat $NMAPTMP | awk '{ print $2 }' ); do
		...
		for _ITM in "${LDNS[@]}"; do
			...
			DNSLOOK=$( dig -x $NMAPIP @$_ITM | grep -EA1 "ANSWER SECTION"|awk /PTR/'{ print $NF; }' | grep -v PTR );
...
echo "$HOSTNAME" | grep -Eiv "template" >/dev/null 2>&1 && id $USERID >/dev/null 2>&1 && rm -f /tmp/.resolv.conf.swp;

# Give a sign that this was actually executed.
